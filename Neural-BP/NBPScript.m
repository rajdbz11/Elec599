...
figure;
h = area(s1Vec, m13Vec/sum(m13Vec), 'FaceColor','b'); hold on
child = get(h,'Children');
set(child,'FaceAlpha',0.5)
h = area(s1Vec, m23Vec/sum(m23Vec), 'FaceColor','r');
child = get(h,'Children');
set(child,'FaceAlpha',0.5)
h = area(s1Vec, ph3Vec/sum(ph3Vec), 'FaceColor','g');
child = get(h,'Children');
set(child,'FaceAlpha',0.5)
h = area(s1Vec, PFinalVec/sum(PFinalVec), 'FaceColor','k');
child = get(h,'Children');
set(child,'FaceAlpha',0.25)

% Adding some baseline spikecounts
baseline    = 100;
M13_Pn      = M13_P + baseline;
M23_Pn      = M23_P + baseline;
RFinal_Pn   = RFinal_P + baseline;

% Get the posterior distributions from these Poisson draws now
m13Vec_Pn    = exp(-0.5*(M13_Pn'*QDNparams.a3_d/DN/norm(QDNparams.a3_d/DN)^2)*s1Vec.^2 + (M13_Pn'*QDNparams.b3_d/DN/norm(QDNparams.b3_d/DN)^2)*s1Vec);
m23Vec_Pn    = exp(-0.5*(M23_Pn'*QDNparams.a3_d/DN/norm(QDNparams.a3_d/DN)^2)*s1Vec.^2 + (M23_Pn'*QDNparams.b3_d/DN/norm(QDNparams.b3_d/DN)^2)*s1Vec);
PFinalVec_Pn = exp(-0.5*(RFinal_Pn'*QDNparams.a3_d/DN/norm(QDNparams.a3_d/DN)^2)*s1Vec.^2 + (RFinal_Pn'*QDNparams.b3_d/DN/norm(QDNparams.b3_d/DN)^2)*s1Vec);

figure;
h = area(s1Vec, m13Vec_Pn/sum(m13Vec_Pn), 'FaceColor','b'); hold on
child = get(h,'Children');
set(child,'FaceAlpha',0.5)
h = area(s1Vec, m23Vec_Pn/sum(m23Vec_Pn), 'FaceColor','r');
child = get(h,'Children');
set(child,'FaceAlpha',0.5)
h = area(s1Vec, ph3Vec/sum(ph3Vec), 'FaceColor','g');
child = get(h,'Children');
set(child,'FaceAlpha',0.5)
h = area(s1Vec, PFinalVec_Pn/sum(PFinalVec_Pn), 'FaceColor','k');
child = get(h,'Children');
set(child,'FaceAlpha',0.25)
